...
AGoal::AGoal(const FObjectInitializer& ObjectInitializer)
{
	...
	// Create AI Sphere
	AIBehaviorCollisionSphere = ObjectInitializer.CreateDefaultSubobject<USphereComponent>(this, "AIBehaviorSphere");
...
void AGoal::BeginPlay()
{
	...
	// Bind to the sphere events
	if (AIBehaviorCollisionSphere)
	{
		AIBehaviorCollisionSphere->OnComponentBeginOverlap.AddUniqueDynamic(this, &AGoal::OnAICollisionSphereBeginOverlap);
		AIBehaviorCollisionSphere->OnComponentEndOverlap.AddUniqueDynamic(this, &AGoal::OnAICollisionSphereEndOverlap);
	}
}
...
EAIBehaviorType AGoal::GetBehaviorType() const
{
	return BehaviorType;
}
...
void AGoal::OnAICollisionSphereBeginOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	OnAISphereOverlap.Broadcast(true, BehaviorType, OtherActor);
}
void AGoal::OnAICollisionSphereEndOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	OnAISphereOverlap.Broadcast(false, BehaviorType, OtherActor);
}