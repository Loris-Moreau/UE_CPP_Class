...
void UGravityGunComponent::BeginPlay()
{
	...
	// Check if I'm the player
	bIsPlayer = Character->GetClass()->IsChildOf(AMainPlayer::StaticClass());
}
...
void UGravityGunComponent::OnTakeObjectInputPressed()
{
	...
	// We've splited this function for the AI
	TryToTakePickUp(RaycastStart, RaycastEnd);
}
...
void UGravityGunComponent::UpdatePickUpLocation()
{
	...
	{
		NewPickUpLocation = (Character->GetActorLocation() + FVector::UpVector * PickUpHeightFromPlayer)
			+ (Character->GetActorForwardVector() * PickUpDistanceFromPlayer);
...
void UGravityGunComponent::ReleasePickUp(bool bThrowPickUp)
{
	...
	if (bThrowPickUp)
	{
		...
		const FVector ImpusleDirection = bIsPlayer ? CameraManager->GetActorForwardVector() : GetOwner()->GetActorForwardVector();
		const FVector Impusle = ImpusleDirection * ThrowForce;
...
bool UGravityGunComponent::TryToTakePickUp(FVector RaycastStart, FVector RaycastEnd)
{
